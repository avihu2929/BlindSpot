...
public class YoloV5Classifier implements Classifier {
    ...
    public static YoloV5Classifier create(
        ...
        context2=context;
        t1=new TextToSpeech(context2, new TextToSpeech.OnInitListener() {
            @Override
            public void onInit(int status) {
                if(status != TextToSpeech.ERROR) {
                    t1.setLanguage(Locale.UK);
                    if (status == TextToSpeech.SUCCESS) {
                        Toast.makeText(context2,"success", Toast.LENGTH_SHORT).show();
                    } else {
                        Log.e("TTS", "Initilization Failed!");
                        Toast.makeText(context2,"fail", Toast.LENGTH_SHORT).show();
                    }
...
//        float[][][] outbuf = new float[1][output_box][labels.size() + 5];
        ...
        t1.setOnUtteranceProgressListener(new UtteranceProgressListener() {
            ...
            public void onStart(String s) {
                Toast.makeText(context2,"start", Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onDone(String s) {
                Toast.makeText(context2,"done", Toast.LENGTH_SHORT).show();
            }
        ...
        for(int i = 0; i <recognitions.size();i++){
            if(!t1.isSpeaking()){
               //screen size 640
                if(recognitions.get(i).getLocation().right<200){
                    t1.speak(recognitions.get(i).getTitle()+" to your left", TextToSpeech.QUEUE_FLUSH, map);
                }
                else if(recognitions.get(i).getLocation().left>440){
                    t1.speak(recognitions.get(i).getTitle()+" to your right", TextToSpeech.QUEUE_FLUSH, map);
                }else{
                    t1.speak(recognitions.get(i).getTitle()+" in front of you", TextToSpeech.QUEUE_FLUSH, map);
                }
            }
            Log.d("Recolo",recognitions.get(i).getLocation().left+ " "+recognitions.get(i).getLocation().right);
            Log.d("Reco",recognitions.get(i).getTitle());
        }
